...
`bridge-optimization` 分支在 `Relate/` 目录下提供了将 Mask 项目的精确肿瘤 mask 桥接到 MAISI 132 类标签的完整管线。

### 管线流程

```
MAISI 生成 CT + 132类mask (不含肿瘤)
    → bridge_maisi_mask.py 在指定器官内画精确肿瘤
    → MAISI ControlNet 按合并mask生成含肿瘤的CT
```

### 快速使用

```bash
# 编辑 Relate/prompts.json 填入 MAISI 输出路径，然后运行
python Relate/bridge_maisi_mask.py

# 验证输出
python Relate/verify_bridge_output.py --merged output.nii.gz --quick
```
...
| 器官 | 命令参数 | 肿瘤标签 | 尺寸档 |
|------|------|:--:|------|
| 肝脏 | `--organ liver` | 26 | tiny/small/medium/large |
| 胰腺 | `--organ pancreas` | 24 | tiny/small/medium |
| 肾脏 | `--organ kidney` | 116 | tiny/small/medium/large |
| 结肠 | `--organ colon` | 27 | tiny/small/medium/large |
| 肺 | `--organ lung` | 23 | tiny/small/medium/large |
| 骨 | `--organ bone` | 128 | tiny/small/medium/large |

详细技术文档见 [Relate/桥接管线技术总结.md](Relate/桥接管线技术总结.md)。
